...
# スプレッド閾値
SPREAD_ENTRY = 0.00220  # 実効スプレッド(100%=1,1%=0.01)がこの値を上回ったらエントリー
...
def get_effective_tick(size_thru, rate_ask, size_ask, rate_bid, size_bid):
    ...
    while True:
        try:
            value = bitbank.fetchOrderBook('MKR/JPY')
            break
        except Exception as e:
            logger.info(e)
            time.sleep(0.1)
...
def limit(side, size, price, param):
    ...
    while True:
        try:
            value = bitbank.create_order('MKR/JPY', type = 'limit', side = side, amount = size, price = price, params = params)
            break
        except Exception as e:
            logger.info(e)
            time.sleep(0.1)
...
def cancel(id):
    ...
    try:
        value = bitbank.cancel_order(symbol = 'MKR/JPY', id = id)
    except Exception as e:
        logger.info(e)
...
def get_status(id):
    ...
    while True:
        try:
            values = bitbank.fetch_open_orders(symbol = 'MKR/JPY')
            break
        except Exception as e:
            logger.info(e)
            time.sleep(0.1)
...
while True:
    ...
    if trade_bid_status == 'open':
        ...
        fetchTrades = bitbank.fetchMyTrades('MKR/JPY', d, 1, {
            'order': 'desc',
        })
